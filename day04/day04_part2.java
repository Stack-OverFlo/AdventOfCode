...
public class day04_part2 {
    ...
    public static final String WORD_TO_FIND = "MAS";
    ...
    public static int countWordOccurrences(char[][] grid) {
        ...
        for (int y = 0; y < rows; y++)
            for (int x = 0; x < cols; x++) {
	       	if ((matches(grid, y, x, 1, 1) ||
		     matches(grid, y + WORD_LENGTH - 1, x + WORD_LENGTH - 1, -1, -1)) && 
		    (matches(grid, y,  x + WORD_LENGTH - 1, 1, -1) ||
		     matches(grid, y + WORD_LENGTH - 1, x, -1, 1)))
		    count++;
            }
        return count;
    }
